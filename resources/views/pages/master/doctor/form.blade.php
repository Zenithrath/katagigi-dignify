<x-app-layout>
    <x-slot:title>{{ $type == 'update' ? __('master.doctor.form.title.edit') : __('master.doctor.form.title.add') }}</x-slot:title>

    <main class="main-table-container">
        <section class="heading">
            <div>
                <h1>{{ $type == 'update' ? __('master.doctor.form.title.edit') : __('master.doctor.form.title.add') }}</h1>
                <p>{{ $type == 'update' ? 'Update doctor information' : 'Register a new doctor' }}</p>
            </div>
        </section>
                ...
                <div class="p-8 flex flex-col">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1">
                        ...
                        <div class="input-group">
                            ...
                            <small class="helper">{{ __('Leave blank if NIPTK/SIP number didn\'t exist.') }}</small>
                            @error('niptk')
                                <small class="danger">{{ $message }}</small>
                            @enderror
                        </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-slate-200 flex justify-end">
                        <button type="submit" class="btn-submit !w-auto !px-8">
                            {{ $type == 'update' ? __('form.buttons.update') : __('form.buttons.create') }}
                        </button>
                    </div>
                </div>
            </div>
