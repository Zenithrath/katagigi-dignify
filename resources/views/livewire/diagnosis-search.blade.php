<div class="w-full">
    ...
    @if (count($selected) > 0)
        <div class="flex flex-wrap gap-2 mb-2">
            @foreach ($selected as $index => $row)
                <span class="inline-flex items-center gap-2 rounded-full bg-emerald-50 border border-emerald-200 pl-3 pr-1.5 py-1 text-sm text-emerald-900">
                    <span class="text-[11px] font-semibold uppercase tracking-wide bg-emerald-700 text-white rounded-full px-2 py-0.5">{{ $row['system'] }}</span>
                    <span class="font-semibold">[{{ $row['code'] }}]</span>
                    <span class="text-emerald-800">{{ $row['display_id'] }}</span>
                    <button type="button" wire:click="remove({{ $index }})" class="rounded-full hover:bg-emerald-200 w-6 h-6 leading-none" aria-label="Hapus kode">&times;</button>
                </span>
    ...
    <div class="relative">
        <input
            ...
            autocomplete="off"
            class="w-full rounded-xl border-slate-200 focus:border-emerald-600 focus:ring-emerald-600 text-sm py-2.5"
        >

        @if (strlen(trim($query)) >= 2)
            <div class="absolute z-20 mt-1 w-full overflow-hidden rounded-xl border border-slate-100 bg-white shadow-lg">
                @forelse ($results as $item)
                    ...
                    >
                        <span class="mt-0.5 shrink-0 text-[11px] font-semibold uppercase tracking-wide rounded-full px-2 py-0.5
